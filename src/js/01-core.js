/* ============================================================
   ...
   ------------------------------------------------------------
   会社で決めた運用ルール（等級・評価の配点・報告の期限など）を、
   そのままデータ定義として持たせている。
   ============================================================ */

var APP_NAME    = '評価制度・組織運営アプリ';
...
var EVAL_ITEMS_MANAGER = [
  ...
  { key:'improve',label:'業務改善・仕組み化',       weight:10,
    desc:'一人しか分からない状態をなくし、手順と記録を残して、次に使えるようにしたか。' }
];
...
var DEFAULT_GRADES = [
  ...
  { code:'G5', expect:'部門成果・予算・重要判断に責任',
    ...
    authority:'部門予算・人員配置・重要判断。会社の方針に関わることは経営層の承認。' }
];
...
var SCORECARD_TEMPLATES = {
  '事業責任者':{
    ...
    reports:'週次KPI会議で数値と対策を報告／大きな数値の悪化は、分かってから24時間以内に経営層へ',
  ...
  '管理職':{
    purpose:'担当チームの成果に責任を持ち、一人ひとりが自分で判断して進められる状態をつくる。',
    responsibilities:['チーム目標とKPIの達成','メンバーの役割と権限の明確化','月次1on1と評価の実施','よくない情報を早く共有すること','業務の手順化と、一人しか分からない状態の解消'],
    ...
    reports:'週次KPI会議で未達と対策を報告／重要な件は24時間以内に経営層へ',
  ...
  '経営幹部':{
    ...
    reports:'週次で全社の数値を共有／重要な件は分かってから24時間以内',
...
var PLAN_WEEKS = [
  ...
  { id:'w3', period:'Week 2〜3', range:[2,3], task:'職種を6〜8区分し役割定義', output:'役割表',
  ...
  { id:'w4', period:'Week 3〜5', range:[3,5], task:'週次KPI会議開始', output:'KPI表・議事記録',
    items:['週次KPI会議の曜日と時間を固定した','KPI表の様式を決めた','45分以内で終える運用にした','未達項目の原因・対策・期限を記録している','経営判断事項を記録している'] },
  ...
  { id:'w7', period:'Week 6〜8', range:[6,8], task:'G1〜G5等級制度作成', output:'等級・昇格基準',
    items:['G1〜G5の期待状態を確定した','全社員を仮格付けした','G4以降の管理職／専門職コースを決めた','昇格の基本条件を明文化した','等級と役割表を紐づけた'] },
...
var MEETING_AGENDA = [
  ...
  { min:5,  label:'経営判断事項と次回確認', hint:'経営層が決めることをはっきりさせ、次回の確認事項を残す。' }
];
...
function emptyData(){
  return {
    ...
    /* --- 決め方・任せ方・お金の使い方の記録 --- */
    decisions:[],        /* 重要な決定（24時間おいてから確定する） */
    ventures:[],         /* 新規案件の1枚企画書（48時間ルール） */
    delegations:[],      /* 任せた仕事（6項目と中間確認） */
